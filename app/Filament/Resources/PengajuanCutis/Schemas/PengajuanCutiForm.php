...
class PengajuanCutiForm
{
                ...
                Section::make('Detail Pengajuan')
                    ->schema([
                        ...
                        Select::make('kelompok_kerja_id')
                            ...
                            ->searchable()
                            ->visible(fn (?PengajuanCuti $record) => ($record ? $record->user : Auth::user())->unitKerja?->jenis === 'operasional')
                            ->required(fn (?PengajuanCuti $record) => ($record ? $record->user : Auth::user())->unitKerja?->jenis === 'operasional')
                            ->live()
                        ...
                        Select::make('jenis_cuti')
                            ...
                            ->searchable()
                            ->required()
                            ->disabled(fn (?PengajuanCuti $record) => $record && (Auth::user()->hasRole('admin') || Auth::user()->hasRole('super_admin')))
                            ->dehydrated(fn (?PengajuanCuti $record) => ! ($record && (Auth::user()->hasRole('admin') || Auth::user()->hasRole('super_admin'))))
                            ->live(),
                        ...
                        DatePicker::make('tanggal_mulai')
                            ...
                            ->required()
                            ->native(false)
                            ->minDate(fn (string $operation, ?PengajuanCuti $record) => match (true) {
                            ...
                            ->rules([
                                fn (string $operation, $get, $record) => function (string $attribute, $value, \Closure $fail) use ($operation, $get, $record) {
                                    ...
                                    if ($shouldValidateTime) {
                                        if (! CutiService::validasiTanggal($date, $konteks)) {
                                    ...
                                    if (CutiService::hitungLamaCuti($date, $date, $unitKerja, $kelompokKerja) === 0) {
                        ...
                        Fieldset::make('Informasi Sisa Saldo Cuti')
                            ->schema([
                                Placeholder::make('sisa_n2')
                                    ->label('Saldo N-2')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['n2']),
                                Placeholder::make('sisa_n1')
                                    ->label('Saldo N-1')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['n1']),
                                Placeholder::make('sisa_n')
                                    ->label('Saldo N')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['n']),
                                Placeholder::make('sisa_besar')
                                    ->label('Cuti Besar')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['besar']),
                                Placeholder::make('sisa_sakit')
                                    ->label('Cuti Sakit')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['sakit']),
                                Placeholder::make('sisa_melahirkan')
                                    ->label('Melahirkan')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['melahirkan']),
                                Placeholder::make('sisa_alasan_penting')
                                    ->label('Alasan Penting')
                                    ->content(fn ($get, ?PengajuanCuti $record) => self::getSimulasiSaldo($get, $record)['penting']),
                            ])
                            ->gridContainer(),
                        ...
                        Fieldset::make('Catatan / Alasan dari Atasan')
                            ->schema([
    ...
    public static function getSimulasiSaldo($get, $record = null): array
    {
        ...
        if ($record && $record->status === 'disetujui' && $lama !== $record->getOriginal('lama_cuti')) {
            $dryRunResult = CutiService::koreksiSaldo($record, $record->getOriginal('lama_cuti'), $lama, true);
        ...
        if ($activeHolds > 0 && $jenis === 'tahunan') {
            ...
            switch ($jenis) {
                case 'besar': $besar = max(0, $besar - $activeHolds);
                    break;
                case 'sakit': $sakit = max(0, $sakit - $activeHolds);
                    break;
                case 'melahirkan': $melahirkan = max(0, $melahirkan - $activeHolds);
                    break;
                case 'alasan_penting': $penting = max(0, $penting - $activeHolds);
                    break;
            }
        }
        ...
        if (! $record || $record->status !== 'disetujui') {
            ...
            if ($jenis === 'tahunan') {
                ...
                switch ($jenis) {
                    case 'besar': $besar -= $lama;
                        break;
                    case 'sakit': $sakit -= $lama;
                        break;
                    case 'melahirkan': $melahirkan -= $lama;
                        break;
                    case 'alasan_penting': $penting -= $lama;
                        break;
                }
            }
        }
